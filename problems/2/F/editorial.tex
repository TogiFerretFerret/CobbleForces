...
Rating: \textbf{4000+ (Legendary Grandmaster)}.
This problem represents the pinnacle of dynamic tree data structures and is likely the hardest problem in the contest.
\begin{itemize}
    \item Standard **Dynamic DP** (via Heavy-Light Decomposition) can handle weight updates but cannot handle structural changes (Link/Cut).
    \item Standard **Link-Cut Trees** (LCT) handle structural changes but are designed for path aggregates. They cannot easily maintain subtree aggregates (like MWIS) because the "dashed edges" in an LCT are not dynamically aggregated in a way that supports non-invertible DP transitions efficiently without complex auxiliary structures.
\end{itemize}
